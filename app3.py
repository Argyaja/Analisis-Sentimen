...
def sentiment_analysis():
    st.title("Perbandingan Model - Model yang Digunakan")
    ...
    with open('modelsvc.sav', 'rb') as f:
    ...
    def predict_sentiment(input_text):
        ...
        prediction_svc = modelsvc.predict(X_vect)
        prediction_lr = modellr.predict(X_vect)

        # Return predictions of all models
        return prediction_xgb, prediction_et, prediction_svc, prediction_lr

    if st.button('Predict'):
        if comment:
            prediction_xgb, prediction_et, prediction_svc, prediction_lr = predict_sentiment(comment)
            ...
            st.write("Model XGBoost prediction:", sentiment_xgb)
            st.write("Model Extra Trees prediction:", sentiment_et)
            st.write("Model Logistic Regression prediction:", sentiment_lr)
        else:
